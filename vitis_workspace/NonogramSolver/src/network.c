...
void network_init(
    ...
    xTaskCreate(
        (lwip_thread_fn)network_startup_task, "startup_task", THREAD_STACKSIZE, NULL,
        DEFAULT_THREAD_PRIO, &startuptask
    );
}

// Created by network_init(). Initialises lwIP, runs DHCP, and once connected, starts the
// application_task_fn that the user provided when they called network_init
int network_startup_task() {
    lwip_init();
    ...
    while (1) {
        ...
        if (server_netif.ip_addr.addr) {                      // Do we have an IP address?
            ...
            xil_printf("\r\n");
            xTaskCreate(
                application_task_fn, "app_task", THREAD_STACKSIZE, NULL, DEFAULT_THREAD_PRIO,
                &apptask
            );
            break;
        }
...
void network_thread(
    ...
    xTaskCreate(
        ...
        DEFAULT_THREAD_PRIO, &rcv_task
    );

    // Start DHCP. This task will now loop forever calling dhcp_fine_tmr and dhcp_coarse_tmr every
    // so often
    xil_printf("\r\nStart DHCP lookup...\r\n");
    dhcp_start(netif);
    while (1) {
